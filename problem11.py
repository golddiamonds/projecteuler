...
#implementation
#		place "string" grid into a int grid list containing 20 lists of 20 numbers.
#			- each set of numbers has a space at the beginning, then a space as a delimitter, with single digits having a 0 appeneded to the front.

#grab 20 numbers at a time and place them into a list

#split grid by ' '
grid_split = grid.rsplit(' ')

#append as ints in sets of 20
grid_int = [[]]
i = 1
j = 0
for num in grid_split:
	grid_int[j].append(int(num))
	if i % 20 == 0:
		grid_int.append([])
		j += 1
	i += 1
